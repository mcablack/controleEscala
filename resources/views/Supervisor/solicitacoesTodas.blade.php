...
<!--    ************** CORRIGIR LISTA DE TROCAS COM DADOS DO OPERADOR DO SUPERVISOR LOGADO***** -->
  ...
  <div class="card card-default">
    ...
    <div class="card-body">
      <table class="table table-striped table-hover table-bordered" id="myTable" width="100%" table-layout:fixed>
        <thead>
          <tr>
            <th style="width: 50px;">Calendário</th>
            <th style="width: 50px;">Matricula</th>
            <th style="width: 250px;">Nome</th>
            <th style="width: 50px;">Data criação</th>
            <th style="width: 50px;">Data troca</th>
            <th style="width: 50px;">Turno escala</th>
            <th style="width: 50px;">Turno troca</th>
            <th align="center" style="width: 50px;">Skill</th>
        ...
        <?php foreach($solicitacoesTodas as  $solictacoes=>$value){?>
          <tr>
            ...
            <td><?= formatarData($value->data_cad_solicitacao ,'d/m/Y')?> </td>
            <td><?= formatarData($value->data_troca,'d/m/Y')?> </td>
            <td><?= $value->horario_inicial_escala_atual.'-'.$value->horario_final_escala_atual ?></td>
            <td><?= $value->horario_inicial_escala_troca.'-'.$value->horario_final_escala_troca ?></td>
            <td><?= $value->desc_skill ?></td>

            <?php if($value->id_status_autorizacao == '1'){ ?>
              <td align="center" >
                <div class="col-md-9">
                  <a style='color: white;'  class="btn btn-warning btn-block" ><i class="fas fa-hourglass-half"></i> &nbsp;  <?= $value->desc_status_autorizacao ?> </a>
                </div>
              </td>
            <?php }elseif($value->id_status_autorizacao == '3'){ ?>
              <td align="center" >
                <div class="col-md-9">
                  <a style='color: white;' class="btn btn-success btn-block"><i class="fas fa-check-circle"></i> &nbsp;<?= $value->desc_status_autorizacao ?>  </a>
                </div>
              </td>
            <?php }elseif($value->id_status_autorizacao == '4'){ ?>
              <td align="center" >
                <div class="col-md-9">
                  <a style='color: white;' class="btn btn-danger btn-block"><i class="fas fa-times-circle"></i> &nbsp;<?= $value->desc_status_autorizacao ?>  </a>
                </div>
              </td>
            <?php } ?>
          </tr>
